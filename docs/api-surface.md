...
- **Freshness on return:** the PWA re-fetches the current read-only view when the tab becomes visible after >60s hidden — tasks created/completed by agents (e.g. the a synced agent client todo sync via MCP) or other devices appear on return without a manual reload. Edit views (task detail, settings, project edit) never auto-refresh, so in-progress form edits are safe.
