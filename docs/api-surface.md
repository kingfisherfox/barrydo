...
- **Home (single page, projects are groups — not a view):** fuzzy search bar (see below), quick-add with title + description inline (optimistic — card appears same-tick, + disabled in flight), then accordion groups: Inbox first, then projects in manual drag order, each collapsible with state remembered per device (localStorage `barrydo_collapsed`). Project groups carry a grip handle — drag to reorder projects (same drag styling as tasks; order persists). Drag a collapsed group's header while dragging to expand it live; drag onto any group header/body (incl. empty groups) to move the task. Every project card renders identically — header + count badge — regardless of task count. Project order is shared truth: `/api/projects` returns drag order (`position`), so Home groups, the quick-add project select, and the task-detail project select all render in that order — after an in-session drag the group order is mirrored live onto the quick-add select (Inbox always first).
...
- **Tap a group header = add a task to that group**: quick-add opens with the project preselected (or Inbox), the options row expanded, and the title focused; fold/unfold moved to the chevron.
- **Fuzzy search** (bar at the top of Home, replaced the old project strip): matches task titles + refs and project names as case-insensitive subsequences (word-start and consecutive-run bonuses, gap penalty). A project-name hit shows all that project's tasks. Rows/groups filter live as you type; collapsed groups force-open while searching; result count shown under the bar; ✕ button or Esc clears. Session-only state — not persisted across reloads. Moving a task by drag-and-drop onto the strip is gone with the strip; use group-to-group drags or the task-detail select.
- **Freshness on return:** the PWA re-fetches the current read-only view when the tab becomes visible after >60s hidden — tasks created/completed by agents (e.g. a synced todo client over MCP) or other devices appear on return without a manual reload. Edit views (task detail, settings, project edit) never auto-refresh, so in-progress form edits are safe.
- **Project edit** lives at `#/project/<id>/edit` (reached via Settings → Projects → Edit): rename, re-describe, delete (tasks → Inbox).
...
## Client wiring
- any MCP client: URL `<your-deployment>/mcp`, header `Authorization: Bearer <API_KEY>` — copy-ready values and JSON config live in the app at Settings → MCP
