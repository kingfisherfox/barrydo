...
Cross-plane writes (a synced agent client todo sync over MCP, other devices) exposed the top false-negative of the sync era: an already-open PWA never re-fetches, so synced tasks look "not created" (live incident 2026-09-08: the synced task existed and was completed server-side while the open app showed nothing). The PWA now re-renders the current read-only view when the tab becomes visible after >60s hidden; edit surfaces (task detail, settings, project edit) are exempt so in-progress form state is never clobbered. Same session, the sync's D2 decision was reversed on the a synced agent client side (edits now push `update_task`; recorded in sync-client's plans/sync-plan.md) — barrydo needed no change; `update_task` was already in the MCP surface.
